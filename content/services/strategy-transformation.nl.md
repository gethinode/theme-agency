---
_schema: default
title: Strategie & transformatie
slug: strategie-transformatie
description:
content_blocks:
  - _bookshop_name: hero
    heading:
      title: Strategie & transformatie
      content: >-
        We helpen met het vormgeven van een aansprekende en tastbare
        digitale strategie die bijdraagt aan de bedrijfsdoelstellingen.
      align: start
      width: 6
    illustration:
      icon: bi star-fill fa-10x
      class: icon-glow text-white text-center
    links:
      - title: Neem contact op
        url: '#!'
        icon: chevron-right
    orientation: horizontal
    cover: true

  - _bookshop_name: about
    heading:
      preheading: Inleiding
      title: Een digitale strategie die werkt
      content: >-
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer ac nisl lobortis, luctus justo sed, scelerisque ex. Donec vitae lectus laoreet, tristique diam non, auctor quam. Aliquam erat volutpat. Donec nulla nibh, aliquet at dignissim in, scelerisque et risus. Nullam scelerisque nec felis vel dictum.
      align: start
    illustration:
      image: /assets/img/jeshoots-com-fzOITuS1DIQ-unsplash.jpg
    links:
      - title: Neem contact op
        url: '#!'
        icon: chevron-right
    order: first
    width: 8

  - _bookshop_name: panels
    heading:
      preheading: Oplossingen
      title: Versnel je digitale transformatie
      content: >-
        Lees meer over hoe we je digitale transformatie vormgeven en versnellen
      align: start
      width: 8
    background:
      color: body-tertiary
    tab-type: underline
    content_style: fs-5
    elements:
      - title: Oplossing 1
        image: /assets/img/nat-9l98kFByiao-unsplash.jpg
        content: |
          Etiam est turpis, sagittis tincidunt sapien vel, tempor dapibus ex.
          Etiam ut nisl ac turpis malesuada maximus non nec ligula. Curabitur
          eget ante ac arcu consequat convallis ac vitae justo. Cras scelerisque
          tristique enim nec laoreet. Aliquam varius vulputate congue. Maecenas
          in lacinia nulla. Duis aliquet, enim non facilisis ornare, felis
          neque fringilla nulla, id porta risus elit sit amet diam.
      - title: Oplossing 2
        image: /assets/img/nat-9l98kFByiao-unsplash.jpg
        content: |
          Nunc sit amet dui sodales, tempus nisl id, vehicula arcu. Fusce
          luctus iaculis ante, non maximus nunc tempus ac. Etiam ac dolor
          erat. Morbi maximus mollis tincidunt. In pretium cursus leo,
          efficitur dictum urna malesuada eu. Orci varius natoque penatibus
          et magnis dis parturient montes, nascetur ridiculus mus.
      - title: Oplossing 3
        image: /assets/img/nat-9l98kFByiao-unsplash.jpg
        content: |
          Nulla maximus at sapien sed varius. In tincidunt, eros dapibus
          consequat sodales, massa velit placerat tortor, sit amet vulputate
          erat ipsum sit amet nunc. Ut ex tortor, egestas quis justo sed, tempus
          pulvinar neque. Aenean mollis consequat justo, et iaculis turpis
          bibendum sit amet. In scelerisque nulla et arcu hendrerit scelerisque.
          Pellentesque in lacus non tellus luctus molestie sit amet id dui.
          Nunc id feugiat lacus. Nullam congue facilisis felis, sit amet tempor
          nunc tincidunt non.

  - _bookshop_name: featured
    heading:
      title: Experttips voor het vormgeven van een digitale strategie
      content: >-
        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod
        tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim
        veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea
        commodo consequat.
      align: start
    illustration:
      image: /assets/img/mediamodifier-OhVnADTC7rY-unsplash.png
      width: 10
    links:
      - title: Download de brochure
        download: '/static/media/example.pdf'
        icon: download
    width: 12

  - _bookshop_name: approach
    heading:
      preheading: Onze aanpak
      title: Van concept tot realisatie in vijf stappen
    illustration:
      image: /assets/img/patrick-perkins-ETRPjvb0KM0-unsplash.jpg
      ratio: 21x9
    cols: 5
    background:
      color: body-tertiary
    orientation: horizontal
    elements:
      - title: Stap 1
        icon: 1-square
        content: >-
          Lorem ipsum dolor sit amet, consectetur adipiscing elit.
      - title: Stap 2
        icon: 2-square
        content: >-
          Curabitur interdum suscipit odio ut fermentum.
      - title: Stap 3
        icon: 3-square
        content: >-
          Sed scelerisque quam vel diam pellentesque scelerisque.
      - title: Stap 4
        icon: 4-square
        content: >-
          Nunc non lacus et odio dapibus commodo.
      - title: Stap 5
        icon: 5-square
        content: >-
          Vivamus convallis aliquam turpis, nec vestibulum lacus.

  - _bookshop_name: articles
    heading:
      preheading: Blog
      title: Recente artikelen
      align: start
      width: 8
    hide_empty: false
    header_style: none
    input:
      section: blog
      reverse: true
      sort: date
      tags:
        - strategie
        - transformatie
    more:
      title: Meer artikelen
      link: /nl/blog/
    cols: 3
    padding: 0
    limit: 3

  - _bookshop_name: cta
    contact: Alex Smith
    background:
      color: primary
      subtle: true
    fluid: false
    links:
      - title: Neem contact op
        url: '#!'
        icon: chevron-right
---
